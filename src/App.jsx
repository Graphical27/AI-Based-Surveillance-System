...
const computeSurvivalProbability = (invaders, defenders, homeAdvantage = 0.1) => {
  ...
  const allRanges = [...invaders, ...defenders].map(f => Math.max(f.R || 0.1, 0.01))
  const avgR = allRanges.reduce((sum, r) => sum + r, 0) / allRanges.length
  
  const effectiveness = ({ count, D, R, M, Re }) => {
    const damage = Math.pow(Math.max(D || 0.1, 0.01), 1.1)
    const range = Math.max(R || 0.1, 0.01) / (Math.max(R || 0.1, 0.01) + avgR)
    const magazine = Math.log(1 + Math.max(M || 0.1, 0.01))
    const recoil = Math.exp(-2 * Math.max(Re || 0, 0))
    const result = count * damage * range * magazine * recoil
    console.log(`Effectiveness for ${count} units:`, { damage, range, magazine, recoil, result })
    return result
  }
  
  const S_inv = invaders.reduce((sum, i) => sum + effectiveness(i), 0)
  const S_def = defenders.reduce((sum, d) => sum + effectiveness(d), 0) * (1 + homeAdvantage)
  
  console.log('Survival calculation:', { S_inv, S_def, total: S_def + S_inv })
  
  const probability = S_def / (S_def + S_inv)
  ...
  const { survivalProbability, threatLevel } = useMemo(() => {
    const calculateSurvivalProbability = () => {
      ...
      Object.entries(counts).forEach(([className, count]) => {
        ...
        if (!stats) {
          console.log(`No stats found for ${className}`)
          return
        }
    ...
    const threat = probability < 0.3 ? 'High' : probability < 0.7 ? 'Medium' : 'Low'
    
    console.log('Final results:', { probability, threat })
    
    return { survivalProbability: probability, threatLevel: threat }
  }, [counts])
      ...
      <div className={`fixed left-0 top-0 h-full w-72 transform transition-all duration-300 z-40 ${
        ...
        <div className="p-8 border-b border-current border-opacity-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm opacity-70">AI Security Monitor</p>
            </div>
          </div>
        </div>
        ...
        <nav className="p-6">
          <ul className="space-y-3">
            ...
            <li className="border-t border-current border-opacity-10 my-6"></li>
            <li>
              ...
              {(originalImage || preview) && (
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {originalImage && (
                      <div>
                        <h5 className="text-md font-medium mb-2 text-gray-600">Original Image</h5>
                        <img 
                          src={originalImage} 
                          alt="Original" 
                    ...
                    {preview && (
                      <div>
                        <h5 className="text-md font-medium mb-2 text-gray-600">Processed Result</h5>
                        <img 
                          src={preview} 
                          alt="Processed" 
                          className="w-full h-auto rounded-xl shadow-lg border border-gray-300 max-h-80 object-contain"
